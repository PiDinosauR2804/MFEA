...
public class Population {
    ...
    public Population(int n, List<Task> tasks) {
        this.nIndividual = n;
...
// initialize
 ...
 //           System.out.println(g);
            ...
            List<Integer> fitnessTa = new ArrayList<>();
            for (Task task : tasks) {
    ...
    boolean checkIndividualVail(List<Integer> c) {
        for (Task t : tasks) {
            if (t.checkIndividualVail(c)) {
                return true;
            }
        }
    ...
    void makeIndividualVail(List<Integer> g) {
        int i = 0;
        int xd = 0;
        while (true) {
            Task t = tasks.get(i);
            if (!t.checkIndividualVail(g)) {
                xd = 0;
                t.makeIndividualVail(g);
            } else {
                xd++;
            }
            if (xd >= tasks.size()) {
